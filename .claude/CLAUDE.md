...
```
@open-inspect/shared  ←  server, web, slack-bot, github-bot, linear-bot
```
...
| `server`      | TypeScript / CF Workers + DO       | Session management, WebSocket streaming, GitHub integration |
...
npm test -w @open-inspect/server          # unit tests (node env)
npm run test:integration -w @open-inspect/server  # integration (workerd/Miniflare + real D1)
...
- **server unit**: co-located as `src/**/*.test.ts` — run in Node environment
- **server integration**: separate `test/integration/*.test.ts` — run in workerd via
...
- **Use seconds for Python, milliseconds for TypeScript.** These match each ecosystem's conventions
  (Modal `timeout=` takes seconds; server uses `_MS` suffixes throughout).
...
- **No `wrangler.toml`**: server config is generated by Terraform, not checked in.
...
- [packages/server/README.md](packages/server/README.md) — API reference, WebSocket protocol, D1
